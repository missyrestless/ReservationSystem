...
// Modified 25-Feb-2011 by Missy Restless - payment handled by Payment script
//
    ...
    if ( aMenu == _MAIN ) {
        ...
        if (Dest == "") {
            ...
            DialogMessage = DialogMessage +
                            "\nFirst select your " + _DESTS + "\n";
            DialogMessage = DialogMessage + _DESTS + " = UNSET";
        }
        else {
            DialogMessage = DialogMessage + "\n" + _DESTS + " = " + Dest;
            if (Day == "") {
...
integer MenuListen( string aMenu, string aButton, string aAvatarName, key aAvatarKey ) {
    ...
    if ( aMenu == _MAIN ) {
        ...
        if(~llListFindList(Dates, [ aButton ])) {
            Day = aButton;
            llMessageLinked(LINK_THIS, 707, Day, "");
        }
    } else if ( aMenu == _TIMES ) {
        if(~llListFindList(Times, [ aButton ])) {
            Time = aButton;
            llMessageLinked(LINK_THIS, 708, Time, "");
        }
    } else if ( aMenu == _DESTS ) {
        if(~llListFindList(Dests, [ aButton ])) {
            Dest = aButton;
            if (ind_days) {
                tmpstr = llList2String(Weekdays, llListFindList(Dests, [Dest]));
                Weekdays = llParseString2List(tmpstr, [","], []);
                Make_Days_List();
            }
            if (ind_times) {
                tmpstr = llList2String(Times, llListFindList(Dests, [Dest]));
                Times = llParseString2List(tmpstr, [","], []);
            }
            llMessageLinked(LINK_THIS, 706, Dest, "");
        }
    }
    return RESHOWDIALOG;
}
...
Set_Texture() {
    ...
    llSetTexture(TEXTURE_TRANSPARENT, 5);
}
                    ...
                    if ( name == "EMAIL" ) {
                        ...
                        if (asperand > 0 && 
                            ~llSubStringIndex(llGetSubString(value,
                                              asperand+2, -1), ".")) {
                            email_address = value;
                            llMessageLinked(LINK_THIS, 704, email_address, "");
                        }
                        else {
                        ...
                        llSetObjectName(value);
                    }
                    else if ( name == "EVENT_TYPE" ) {
                        _DESTS = value;
                        _NavigationMenus = [ _MAIN, _DATES, _DESTS, _TIMES ];
                    }
                    else if ( name == "EVENTS" ) {
                        Dests = llParseString2List(value, [","], []);
                        llMessageLinked(LINK_THIS, 700,
                                        llDumpList2String(Dests, ","), "");
                    }
                    else if ( name == "PRICES" ) {
                        Prices = llParseString2List(value, [","], []);
                        llMessageLinked(LINK_THIS, 701,
                                        llDumpList2String(Prices, ","), "");
                    }
                    else if ( name == "DEFAULT_PRICE" ) {
                        defprice = (integer)value;
                        llMessageLinked(LINK_THIS, 702, value, "");
                    }
                    else if ( name == "DAYS" ) {
                        if (value != "") {
                            Weekdays = llParseString2List(value, ["|"], []);
                            if (llGetListLength(Weekdays) > 1)
                                ind_days = 1;
                            else
                                Weekdays = llParseString2List(value, [","], []);
                        }
                        ...
                        llMessageLinked(LINK_THIS, 703, value, "");
                    }
                }
                NotecardLine++;
                D_QueryID = llGetNotecardLine( CONFIG_CARD, NotecardLine );
...
state getinfo {
    state_entry() {
        ...
        if (user_key == NULL_KEY)
            ...
            llMessageLinked(LINK_THIS, 705, "", user_key);
    ...
    {
        if (num == 800) { // Underpayment with debit, start over
            state default;
        }
        else if (num == 888) { // Reservation booked
            state default;
        }
        else if (num == 999) { // Payment window timed out
            state default;
        }
    }
    ...
    timer() {
        ...
        if ((Day != "") && (Time != "") && (Dest != "")) {
            llMessageLinked(LINK_THIS, 777, Dest, "");
            llSetTimerEvent(0.0);
        }
    }
